...
Нажав на кнопку '2D -> 3D' можно получить 3D изображение вычисленного решения.\
В 3D режиме перемещения мыши при нажатой левой кнопке приводят к изменению проекции 3D изображения.\
А если ещё и зажать Shift, то будет смещаться всё изображение.

Программа написана на C#(WPF).\
В OpenCL используется Cloo, в CUDA - ManagedCUDA.\
Точность всех методов $O(h^{2})$

Готовый исполняемый файл находится в каталоге VisualLaplacePoisson2D\bin\x64\Release\net9.0-windows\
Некоторые анимированые Gif находятся в каталоге ResultPictures
...
13. Эволюционной факторизации

## Прямые методы
14. Циклической(полной) редукции(CR)(2 варианта)
15. Разделения переменных(FA)(4 варианта, 2 основных - FFT и прогонка, 2 дополнительных - редукция и маршевый скалярный)
16. Неполной редукции(FACR)
17. Матричной прогонки
18. Маршевый(GMA)(параметрический)
...
5. Эволюционная факторизация - сверхбыстрый счёт на установление.
...
Для битностей 128 и 256 за основу был взят NetQD, который в свою очередь основан на https://github.com/aoki-t/QD, http://web.mit.edu/tabbott/Public/quaddouble-debian/qd-2.3.4-old/docs/qd.pdf.
Добавил туда поддержку интерфейса INumber и сопутствующие ему(ITrigonometricFunctions, ILogarithmicFunctions и т.д.).
Сейчас DD128, QD256 лежат на ГитХабе отдельным проектом https://github.com/NickUtenkov/DD128_QD256_INumber
...
Многосеточный примерно равен $(1 + 1/4 + 1/16 + 1/32 + ...) * 3$
Эволюционной факторизации - 3\
