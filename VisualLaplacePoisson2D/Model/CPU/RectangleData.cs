...
{
	internal struct RectangleData<T> where T : unmanaged, INumber<T>, ITrigonometricFunctions<T>, IPowerFunctions<T>, IExponentialFunctions<T>, IHyperbolicFunctions<T>, ILogarithmicFunctions<T>, IRootFunctions<T>, IMinMaxValue<T>
	{
		...
		public void recalculateSteps(string strXMin, string strXMax, int cXSegmentsIn, string strYMin, string strYMax, int cYSegmentsIn, PlatformAndSchemeIndex platformScheme, bool isVarSepProgonkaGPU, VarSepMethodsEnum varSepMethodCPU)
		{
			...
			calculator.setExpression(strXMin);
			xMin = calculator.Calculate();
			calculator.setExpression(strXMax);
			xMax = calculator.Calculate();
			stepX = (xMax - xMin) / T.CreateTruncating(cXSegments);

			calculator.setExpression(strYMin);
			yMin = calculator.Calculate();
			calculator.setExpression(strYMax);
			yMax = calculator.Calculate();
			stepY = (yMax - yMin) / T.CreateTruncating(cYSegments);
		}
	}

	struct RectangleDataDouble
	{
