...
{
	class RelaxationScheme<T> : Iterative2DScheme<T>, IScheme<T> where T : unmanaged, INumber<T>, ITrigonometricFunctions<T>, ILogarithmicFunctions<T>, IRootFunctions<T>, IMinMaxValue<T>, IPowerFunctions<T>, IExponentialFunctions<T>, IHyperbolicFunctions<T>
	{
		T[,] fn;
		...
		public RelaxationScheme(RectangleData<T> rectData, Func<T, T, T> fKsi, bool isSeidel, bool isChebyshIn, T eps) :
			base(rectData.xMin, rectData.yMin, rectData.stepX, rectData.stepY)
		{
			...
			else
			{
				...
				if (equalSteps) GridIterator.iterate(rectData.cXSegments, rectData.cYSegments, (i, j) => fn[i, j] = stepX2 * fKsi(xMin + stepX * T.CreateTruncating(i), yMin + stepX * T.CreateTruncating(j)));
				else GridIterator.iterate(rectData.cXSegments, rectData.cYSegments, (i, j) => fn[i, j] = fKsi(xMin + stepX * T.CreateTruncating(i), yMin + stepY * T.CreateTruncating(j)));
			}
