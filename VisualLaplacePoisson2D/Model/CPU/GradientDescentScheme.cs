...
{
	class GradientDescentScheme<T> : Iterative2DScheme<T>, IScheme<T> where T : unmanaged, INumber<T>, ITrigonometricFunctions<T>, ILogarithmicFunctions<T>, IRootFunctions<T>, IMinMaxValue<T>, IPowerFunctions<T>, IExponentialFunctions<T>, IHyperbolicFunctions<T>
	{//steepest descent
		T stepX2, stepY2, eps;
		T[,] un1;
		T[,] fn;
		...
		public GradientDescentScheme(RectangleData<T> rectData, Func<T, T, T> fKsi, T eps) :
			base(rectData.xMin, rectData.yMin, rectData.stepX, rectData.stepY)
		{
			...
			else
			{
				...
				GridIterator.iterate(fn.GetUpperBound(0), fn.GetUpperBound(1), (i, j) => fn[i, j] = fKsi(xMin + stepX * T.CreateTruncating(i), yMin + stepY * T.CreateTruncating(j)));
			}
