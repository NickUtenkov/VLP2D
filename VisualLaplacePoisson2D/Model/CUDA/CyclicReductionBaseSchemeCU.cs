...
{
	class CyclicReductionBaseSchemeCU<T> : Direct1DBoundariesScheme<T>, IScheme<T> where T : unmanaged, INumber<T>, IMinMaxValue<T>, ITrigonometricFunctions<T>, ILogarithmicFunctions<T>, IPowerFunctions<T>, IExponentialFunctions<T>, IHyperbolicFunctions<T>, IRootFunctions<T>
	{
		protected T bCoef;
		Func<T, T, T> fKsi;
		...
		public CyclicReductionBaseSchemeCU(RectangleData<T> rectData, Func<T, T, T> fKsiIn, List<BitmapSource> lstBitmap, Func<bool, MinMaxF, Adapter2D<float>, BitmapSource> fCreateBitmap, Action<double> reportProgressIn, int cudaDevice) :
		...
		protected void initRigthHandSide(T[] fj)
		{
			if (fKsi != null)
			{
				...
				int upper1 = N1;
				int upper2 = N2;
				GridIterator.iterate(upper1, upper2, (i, j) => fj[i * dim2 + j] = stepX2 * fKsi(xMin + stepX * T.CreateTruncating(i), yMin + stepY * T.CreateTruncating(j)));//[SNR] p.123 (8)
			}
		}
