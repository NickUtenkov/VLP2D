...
namespace WPFChart3D
{
    public class TextureMapping
    {
            ...
            unsafe
            {
                ...
                for (int nY = 0; nY < 64; nY++)
                {
                    for (int nX = 0; nX < 64; nX++)
                    {
                        ...
                        Color color = PaletteColor(palNoPurple, k);

						      * (pStart + nY * nL + nX * 3 + 0) = (byte)(color.B);
                        *(pStart + nY * nL + nX * 3 + 1) = (byte)(color.G);
                        *(pStart + nY * nL + nX * 3 + 2) = (byte)(color.R);
						      //for Bgr32
						      /**(pStart + nY * nL + nX * 4 + 0) = (byte)(color.B);
						      *(pStart + nY * nL + nX * 4 + 1) = (byte)(color.G);
						      *(pStart + nY * nL + nX * 4 + 2) = (byte)(color.R);
						      *(pStart + nY * nL + nX * 4 + 3) = 0;*/
					}
				}
		...
		static public Color PaletteColor(BitmapPalette pal, double val)
		{
			double maxPalEntryValue = pal.Colors.Count - 1;
			double idx = maxPalEntryValue * val;
			return pal.Colors[(byte)idx];
		}

		static public Color PaletteColor(BitmapPalette pal, double val, double zMin, double zMax)
         {
			   double maxPalEntryValue = pal.Colors.Count - 1;
			   double df = zMax - zMin;
			   double idx = maxPalEntryValue * (val - zMin) / df;
            return pal.Colors[(byte)idx];
   		}
		...
		/*static public Color PaletteColor(BitmapPalette pal, double val, double zMin, double zMax)
		{
			double maxPalEntryValue = pal.Colors.Count - 1;
			double df = zMax - zMin;
			double idx = !(val == float.NaN) ? 1 + (maxPalEntryValue - 1) * (val - zMin) / df : 0;
			return pal.Colors[(byte)idx];
		}*/
	}
}
